...
import {ITagView, TagsViewStore} from "@/store/modules/tags-view";
  ...
  setup() {
    ...
    const state = reactive({
      ...
      refreshSelectedTag: (view: any) => {
        TagsViewStore.delCachedView(<ITagView>view)
        const {fullPath} = view
        nextTick(() => {
      ...
      closeSelectedTag: (view: any) => {
        ...
        if (state.isActive(view)) {
          toLastView(TagsViewStore.visitedViews, <ITagView>view)
        }
      },
      closeOthersTags: () => {
        ...
        TagsViewStore.delOthersViews(state.selectedTag as ITagView)
      },
